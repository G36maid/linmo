...
EOF
	...
	if [ -n "$gnu_functional_criteria" ] || [ -n "$llvm_functional_criteria" ]; then
		...
		all_criteria=""
		[ -n "$gnu_functional_criteria" ] && all_criteria="$all_criteria $(echo "$gnu_functional_criteria" | tr ' ' '\n' | cut -d= -f1)"
		[ -n "$llvm_functional_criteria" ] && all_criteria="$all_criteria $(echo "$llvm_functional_criteria" | tr ' ' '\n' | cut -d= -f1)"

		for criteria in $(echo "$all_criteria" | tr ' ' '\n' | sort -u); do
			gnu_status=$(echo "$gnu_functional_criteria" | tr ' ' '\n' | grep "^${criteria}=" | cut -d= -f2 || echo "")
			llvm_status=$(echo "$llvm_functional_criteria" | tr ' ' '\n' | grep "^${criteria}=" | cut -d= -f2 || echo "")
